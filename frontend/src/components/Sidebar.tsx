...
// Navigation cible Yiri 2 (cahier des charges §29). "Saison" et "Sessions" /
// "Paramètres" ne sont pas encore des pages à part entière — pas de fichier
// vide juste pour la forme, ils rejoindront la nav quand ils existeront
// réellement (Saison : Phase E ; Sessions/Paramètres : hors périmètre Mission 1).
const NAV_ITEMS = [
  { id: 'today', label: "Aujourd'hui", icon: '◈', path: '/today', dispo: true },
  { id: 'constructions', label: 'Constructions', icon: '⌁', path: '/constructions', dispo: true },
  { id: 'trajectory', label: 'Trajectoire', icon: '◒', path: '/trajectory', dispo: true },
  { id: 'reflections', label: 'Réflexions', icon: '◷', path: '/reflections', dispo: true },
];

const NAV_ITEMS_SECONDAIRES = [
  { id: 'reminders', label: 'Rappels', icon: '🔔', path: '/reminders', dispo: true },
];
  ...
  return (
    <aside className="app-sidebar" style={styles.aside}>
      ...
      <nav className="sidebar-nav" style={styles.nav}>
        ...
        <div className="sidebar-divider" style={styles.navDivider} />

        {NAV_ITEMS_SECONDAIRES.map((item) => (
          <button
            key={item.id}
            className="sidebar-nav-btn"
            style={{
...
const styles: Record<string, React.CSSProperties> = {
  ...
  navDivider: { height: 1, background: 'var(--line)', margin: '6px 4px' },

...
// Mission 5 : 'termine' ajouté pour porter la sémantique Saison (voir
// models/programme.py, StatutProgramme).
export type StatutProgramme = 'actif' | 'archive' | 'termine';
...
export interface Axe {
  id_axe: number;
  nom: string;
  // Mission 5 : nullable — une Construction moderne rattachée à cette
  // Saison peut ne pas avoir ces notions Wakati (voir schemas/axe.py).
  phase_deverrouillage: number | null;
...
// Mission 4 — vocabulaire moderne de décision, coexiste avec DecisionBilan.
export type TypeDecision =
  | 'continuer' | 'modifier' | 'reduire' | 'suspendre'
  | 'abandonner' | 'approfondir' | 'ne_rien_changer';
...
export interface Bilan {
  ...
  decision: DecisionBilan | null;
  type_decision: TypeDecision | null;
...
// Mission 4 — flux libre : ni id_programme ni semaine ne sont obligatoires.
export interface ReflexionCreate {
  id_programme?: number | null;
  semaine?: number | null;
  id_axe?: number | null;
  remarque?: string | null;
...
// Mission 4 — flux moderne, sans programme dans l'URL.
export const creerReflexion = (payload: ReflexionCreate): Promise<Bilan> =>
  apiRequest('/bilans', { method: 'POST', body: payload });

export const listerMesReflexions = (): Promise<Bilan[]> => apiRequest('/bilans');

export const getReflexion = (id: number): Promise<Bilan> => apiRequest(`/bilans/${id}`);

...
def progres_par_pilier(
    ...
    # Mission 5 : une Construction rattachée à cette Saison peut ne pas avoir
    # de pilier (§11 — catégorisation facultative, plus une des "quatre
    # dimensions obligatoires"). Exclue de cette agrégation par pilier,
    # jamais forcée dans une case arbitraire.
    axes_deverrouilles = [
        a for a in axes if a.pilier is not None and axe_est_deverrouille(programme, a, semaine)
    ]
...
def _get_axe_ou_404(axe_id: int, current_user: Utilisateur, db: Session) -> Axe:
    """
    Anti-IDOR — deux façons d'être propriétaire, comme _get_bilan_ou_404
    (Mission 4) : id_utilisateur direct (toute Construction créée depuis
    Mission 5, avec ou sans Saison), ou pour un Axe historique qui n'a pas
    cette colonne renseignée, déduit via Axe → Programme → Utilisateur.

    Mission 5 : cette fonction utilisait un simple INNER JOIN vers Programme,
    qui excluait silencieusement toute Construction sans Saison (id_programme
    NULL) — y compris pour son propriétaire légitime. Bug réel, corrigé ici
    plutôt que dans chaque appelant, puisque cette fonction est réutilisée
    telle quelle par Engagement (routers/engagements.py) et Observation
    (routers/observations.py) : sans ce correctif, une Construction sans
    Saison ne pouvait recevoir ni Engagement ni Observation.
    """
    axe = db.query(Axe).filter(Axe.id_axe == axe_id).first()
    if axe is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Axe introuvable")

    est_proprietaire = False
    if axe.id_utilisateur is not None:
        est_proprietaire = axe.id_utilisateur == current_user.id_utilisateur
    elif axe.programme is not None:
        est_proprietaire = axe.programme.id_utilisateur == current_user.id_utilisateur
